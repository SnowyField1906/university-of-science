...
public:
    ...
    DiffieHellman(biguint p, biguint g, biguint alice, biguint bob)
    {
        this->p = p;
        this->g = g;
        this->alice = alice;
    ...
    // HELPER METHODS ----------------------------------------------------------------------
    string alice_public_key() const // Compute public key of Alice
    {
        return g.pow(alice, p).to_hex(false);
    }

    string bob_public_key() const // Compute public key of Bob
    {
        return g.pow(bob, p).to_hex(false);
    }

    string shared_key() const // Compute shared key
    {
        return g.pow(alice * bob, p).to_hex(false);
    }
...
void file_processing(string input, string output)
{
    ...
    inputFile >> a_hex;
    biguint a;
    a.from_hex(a_hex, false);

    string b_hex;
    inputFile >> b_hex;
    biguint b;
    b.from_hex(b_hex, false);

    cout << "p-value (decimal): " << p << endl;
    cout << "g-value (decimal): " << g << endl;
    cout << "a-value (decimal): " << a << endl;
    cout << "b-value (decimal): " << b << endl;

    stringstream buffer;
    DiffieHellman dh = DiffieHellman(p, g, a, b);
    buffer << dh.alice_public_key() << endl
           << dh.bob_public_key() << endl
           << dh.shared_key() << endl;
