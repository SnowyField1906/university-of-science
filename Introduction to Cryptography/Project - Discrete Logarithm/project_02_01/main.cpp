...
public:
    ...
    DiscreteLogarithm(biguint p, biguint g)
    {
        this->p = p;
        this->g = g;
    }
    ...
    // HELPER METHODS ----------------------------------------------------------------------
    bool is_primitive_root(vector<biguint> factors) const // Check if primitive root
    {

        biguint p_minus_one = p - biguint(1);

        for (auto factor : factors)
        {
            if (g.pow((p_minus_one) / factor, p) == biguint(1))
            {
                return false;
            }
        }
...
void file_processing(string input, string output)
{
    ...
    for (int i = 0; i < n.to_int(); i++)
    {
        string factor_hex;
        biguint factor;
        inputFile >> factor_hex;
        factor.from_hex(factor_hex, false);
    ...
    cout << "factor-values (decimal): ";
    for (auto factor : factors)
    {
        cout << factor << " ";
    }
    cout << endl
         << "g-value (decimal): " << g << endl;

    stringstream buffer;
    DiscreteLogarithm dl = DiscreteLogarithm(p, g);
    buffer << (dl.is_primitive_root(factors) == true ? "1" : "0");
