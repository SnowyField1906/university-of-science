...
public:
    ...
    ElGamal()
    {
        p = biguint(0);
        g = biguint(0);
        secret_exponent = biguint(0);
    }

    ElGamal(biguint p, biguint g, biguint secret_exponent)
    {
        this->p = p;
        this->g = g;
        this->secret_exponent = secret_exponent;
    }
    ...
    // HELPER METHODS ----------------------------------------------------------------------
    string public_exponent() // Get public exponent
    {
        return (g.pow(secret_exponent, p)).to_hex(false);
    }

    string message(const biguint &cipher1, const biguint &cipher2) // Get message
    {
        biguint s = cipher1.pow(secret_exponent, p);
        biguint s_inv = s.mod_inverse(p);
        return (cipher2 * s_inv % p).to_hex(false);
    }
...
void file_processing(const string &input, const string &output)
{
    ...
    inputFile >> c1_hex;
    biguint c1;
    c1.from_hex(c1_hex, false);

    string c2_hex;
    inputFile >> c2_hex;
    biguint c2;
    c2.from_hex(c2_hex, false);

    cout << "p-value (decimal): " << p << endl;
    cout << "g-value (decimal): " << g << endl;
    cout << "x-value (decimal): " << x << endl;
    cout << "c1-value (decimal): " << c1 << endl;
    cout << "c2-value (decimal): " << c2 << endl;

    stringstream buffer;
    ElGamal eg = ElGamal(p, g, x);
    buffer << eg.public_exponent() << endl
           << eg.message(c1, c2) << endl;
