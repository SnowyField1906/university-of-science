...
#define TIMEOUT chrono::seconds(60) // Timeout in seconds
...
public:
    ...
    biguint mul_mod(const biguint &rsh, const biguint &modulo) const // Modular multiplication
    {
        biguint res = biguint(0);
        biguint mult = *this;
        biguint right = rsh;
        biguint mod = modulo;

        while (right != biguint(0))
        {
            if (right.data[0] % 2 == 1)
            {
                res += mult;
...
public:
    ...
    ElGamal()
    {
        p = biguint(0);
        g = biguint(0);
        public_exponent = biguint(0);
    }

    ElGamal(biguint p, biguint g, biguint public_exponent)
    {
        this->p = p;
        this->g = g;
    ...
    // HELPER METHODS ----------------------------------------------------------------------
    string secret_exponent() // Get secret exponent
    {
        biguint res = biguint(0);
        biguint cur_pow = biguint(1);
        biguint cur_mult = g;

        while (cur_mult != public_exponent)
        {
            cur_mult *= g;
            cur_mult %= p;
            cur_pow++;
        ...
        return cur_pow.to_hex(false);
    }

    bool verify(biguint message, biguint signature1, biguint signature2) // Verify signature
    {
        biguint left = g.pow(message, p);
        biguint right = public_exponent.pow(signature1, p) * signature1.pow(signature2, p) % p;
...
// Process file
void file_processing(const string &input, const string &output)
{
    // set timeout
    clock_t begin = clock();
    ...
    inputFile >> g_hex;
    biguint g;
    g.from_hex(g_hex, false);

    string y_hex;
    inputFile >> y_hex;
    biguint y;
    y.from_hex(y_hex, false);

    string m_hex;
    inputFile >> m_hex;
    biguint m;
    m.from_hex(m_hex, false);

    string r_hex;
    inputFile >> r_hex;
    biguint r;
    r.from_hex(r_hex, false);

    string h_hex;
    inputFile >> h_hex;
    biguint h;
    h.from_hex(h_hex, false);

    cout << "p-value (decimal): " << p << endl;
    cout << "g-value (decimal): " << g << endl;
    cout << "y-value (decimal): " << y << endl;
    cout << "m-value (decimal): " << m << endl;
    cout << "r-value (decimal): " << r << endl;
    cout << "h-value (decimal): " << h << endl;

    stringstream buffer;
    ElGamal eg = ElGamal(p, g, y);
    buffer << (eg.verify(m, r, h) ? "1" : "0");
